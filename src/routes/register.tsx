...
function RegisterPage() {
  ...
  async function verifierIdentite() {
    ...
    if (!data) { setError("Informations introuvables dans la liste officielle de votre établissement."); return; }
    setStep(4);
  }
  ...
  async function creerCompte() {
    ...
    try {
      await finalizeStudentSignup({ data: {
        etablissement_id, filiere_id, niveau_id, nom_complet, date_naissance,
      }});
      setStep(5);
    } catch (e: unknown) {
    ...
    <div className="min-h-screen bg-muted/40">
      ...
      <div className="mx-auto max-w-xl px-6 py-10">
        <div className="mb-8 flex items-center justify-between">
          <Logo />
          <Link to="/login" className="text-xs font-medium text-muted-foreground hover:text-foreground">Déjà inscrit ? Se connecter</Link>
        </div>
        ...
        <div className="rounded-3xl border border-border bg-surface p-8 shadow-card">
          {step===1 && (
            <>
              ...
              <select value={etablissement_id} onChange={e=>setEtab(e.target.value)} className="mt-6 w-full rounded-xl border border-input bg-background px-4 py-3 text-sm">
                <option value="">— Choisir —</option>
                {etabs.map(e => <option key={e.id} value={e.id}>{e.nom}</option>)}
              </select>
              {etabs.length===0 && <p className="mt-2 text-xs text-muted-foreground">Aucun établissement partenaire actif pour le moment.</p>}
              <button disabled={!etablissement_id} onClick={()=>setStep(2)} className="mt-6 w-full rounded-xl bg-primary py-3 text-sm font-semibold text-primary-foreground disabled:opacity-50">Continuer</button>
            </>
          )}
          ...
          {step===2 && (
            <>
              ...
              <div className="mt-6 space-y-3">
                <select value={filiere_id} onChange={e=>setFiliere(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 text-sm">
                  <option value="">— Filière —</option>
                  {filieres.map(f => <option key={f.id} value={f.id}>{f.nom}</option>)}
                </select>
                <select value={niveau_id} onChange={e=>setNiveau(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 text-sm">
                  <option value="">— Niveau —</option>
                  {niveaux.map(n => <option key={n.id} value={n.id}>{n.nom}</option>)}
                </select>
              </div>
              <div className="mt-6 flex gap-3">
                <button onClick={()=>setStep(1)} className="rounded-xl border border-border px-4 py-3 text-sm font-medium">Retour</button>
                <button disabled={!filiere_id||!niveau_id} onClick={()=>setStep(3)} className="flex-1 rounded-xl bg-primary py-3 text-sm font-semibold text-primary-foreground disabled:opacity-50">Continuer</button>
              </div>
            </>
          )}
          ...
          {step===3 && (
            <>
              <p className="text-xs font-semibold uppercase tracking-widest text-terracotta">Étape 3 / 4</p>
              <h1 className="mt-1 font-display text-2xl font-bold">Vérification d'identité</h1>
              <p className="mt-1 text-sm text-muted-foreground">Ces informations seront comparées à la liste officielle importée par votre établissement.</p>
              <div className="mt-6 space-y-3">
                <input value={nom_complet} onChange={e=>setNom(e.target.value)} placeholder="Nom complet" className="w-full rounded-xl border border-input bg-background px-4 py-3 text-sm" />
                <input type="date" value={date_naissance} onChange={e=>setDate(e.target.value)} className="w-full rounded-xl border border-input bg-background px-4 py-3 text-sm" />
              </div>
              {error && <p className="mt-3 text-xs text-red-600">{error}</p>}
              <div className="mt-6 flex gap-3">
                <button onClick={()=>setStep(2)} className="rounded-xl border border-border px-4 py-3 text-sm font-medium">Retour</button>
                <button disabled={!nom_complet||!date_naissance||busy} onClick={verifierIdentite} className="flex-1 rounded-xl bg-primary py-3 text-sm font-semibold text-primary-foreground disabled:opacity-50">
          ...
          {step===4 && (
            <>
              ...
              <h1 className="mt-1 font-display text-2xl font-bold">Créez votre compte</h1>
              <p className="mt-1 text-sm text-muted-foreground">Utilisez une adresse Gmail pour recevoir vos notifications.</p>
              <div className="mt-6 space-y-3">
          ...
          {step===5 && (
            <div className="text-center">
              ...
              <Link to="/app" className="mt-6 inline-flex rounded-xl bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground">Ouvrir mon espace</Link>
            </div>
          )}
        </div>
      </div>
