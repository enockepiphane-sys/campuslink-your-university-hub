...
type Etudiant = { nom_complet:string; etablissement_id:string };
...
function Home() {
  ...
  useEffect(() => {
    if (!auth.user) return;
    (async () => {
      const { data: e } = await supabase.from("etudiants").select("nom_complet,etablissement_id").eq("user_id", auth.user!.id).maybeSingle();
      setEtu(e);
      if (e?.etablissement_id) {
        const { data: et } = await supabase.from("etablissements").select("nom").eq("id", e.etablissement_id).maybeSingle();
        setEtab(et);
        const { data: a } = await supabase.from("annonces").select("id,titre,contenu,tag,urgent,created_at").eq("etablissement_id", e.etablissement_id).order("created_at",{ascending:false}).limit(3);
        setAnn(a ?? []);
